...
import { EntityModel, Model } from "./model";

jsep.plugins.register(jsepTemplateLiteral);
jsep.plugins.register(jsepObject);
jsep.plugins.register(jsepArrow);

export function compileQuery<T extends object, TArgs extends [...TArgs], R>(
  entity: EntityModel,
  builder?:
    | ((...args: [QueryBuilder<T>, ...TArgs]) => QueryBuilder<R>)
    | ((builder: QueryBuilder<T>) => QueryBuilder<R>)
) {
  ...
  if (builder) {
    ...
    node = builder(new QueryBuilder(node), ...([] as unknown as TArgs)).node;
  }
...
class QueryCompiler implements QueryVisitor<SqlNode> {
  ...
  visitSelect(select: SelectNode): SqlNode {
    ...
    try {
      ...
      const sql = new SqlSelect(projection, from);
  ...
  private static mapOp(op: Operator): SqlOperator {
    switch (op) {
      case Operator["=="]:
        return SqlOperator["="];
      case Operator["!="]:
